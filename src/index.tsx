...
function getStyle(
	...
	switch (place) {
		case 'top':
			arrowStyle.transform = `translate(${
				(tipWidth - arrowSize) / 2
			}px, ${-arrowSize / 2}px) rotate(0.375turn)`
			break
		case 'bottom':
			arrowStyle.transform = `translate(${
				(tipWidth - arrowSize) / 2
			}px, ${-tipHeight + arrowSize / 2}px) rotate(-0.125turn)`
			break
		case 'left':
...
function Tooltip({
	...
	;(function changeCss() {
		if (
			...
			const row0 =
				Math.max(
					parent.top - tipHeight + arrowSize - arrowSize / SQRT2,
					0,
				) + 'px'
			const row1 =
			...
			const row2 =
				Math.min(
					parent.bottom + arrowSize / SQRT2,
					viewHeight - tipHeight,
				) + 'px'
			const col0 =
